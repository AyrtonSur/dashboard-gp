...
st.set_page_config(
    page_title="Dashboard PS - IN Junior",
    page_icon="📊",
    layout="wide"
)
...
# Filtro de Raça/Cor
racas = ["Todas"] + sorted(df["Como você se autodeclara em relação à sua cor ou raça?  "].unique().tolist())
...
if "Todos" not in periodo_selecionado and len(periodo_selecionado) > 0:
    df_filtrado = df_filtrado[df_filtrado["Qual o seu período?"].isin(periodo_selecionado)]

if "Todas" not in raca_selecionada and len(raca_selecionada) > 0:
    df_filtrado = df_filtrado[df_filtrado["Como você se autodeclara em relação à sua cor ou raça?  "].isin(raca_selecionada)]
...
with col3:
    taxa_entrevista = (entrevistas / len(df_filtrado) * 100) if len(df_filtrado) > 0 else 0
    st.metric("Taxa de Entrevista", f"{taxa_entrevista:.1f}%")

with col4:
    lgbtq = len(df_filtrado[df_filtrado["Você se identifica como parte da comunidade LGBTQIAPN+?  "] == "Sim"])
...
with tab1:
    ...
    responderam = len(df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta", na=False)])
    
    # Fase 2: Passaram Fit Cultural (excluindo eliminados do fit cultural)
    passaram_fit_cultural = len(df_filtrado[
        ~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural", na=False, regex=True)
    ])
    
    # Fase 3: Compareceram à Dinâmica (excluindo falta na dinâmica)
    compareceram_dinamica = len(df_filtrado[
        ~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica", na=False, regex=True)
    ])
    
    # Fase 4: Aprovados na Dinâmica (excluindo eliminados da dinâmica)
    aprovados_dinamica = len(df_filtrado[
        ~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica|Eliminados da Dinâmica", na=False, regex=True)
    ])
    ...
    fig1 = go.Figure(go.Funnel(
        y=['Inscritos', 'Responderam', 'Aprovados<br>Fit Cultural', 'Compareceram<br>Dinâmica', 'Aprovados<br>Dinâmica', 'Entrevista'],
        x=[total_candidatos, responderam, passaram_fit_cultural, compareceram_dinamica, aprovados_dinamica, entrevista],
        textposition="inside",
        textinfo="value+percent initial",
        marker=dict(
            color=['#3498DB', '#5DADE2', '#85C1E9', '#7DCEA0', '#52BE80', '#2ECC71'],
    ...
    with col3:
        ...
        fase_counts_ordenado = pd.Series({fase: fase_counts.get(fase, 0) for fase in ordem_fases if fase in fase_counts.index})
        
        # Cores personalizadas para cada tipo de fase
        cores_fase = {
            'Eliminados Falta de Resposta': '#E74C3C',  # Vermelho
            'Eliminados do Fit Cultural e Vídeo de Apresentação': '#C0392B',  # Vermelho escuro
            'Falta na Dinâmica': '#F39C12',  # Amarelo
            'Eliminados da Dinâmica': '#E67E22',  # Laranja
            'Entrevista': '#2ECC71'  # Verde para sucesso
        }
        
        cores = [cores_fase.get(fase, '#95A5A6') for fase in fase_counts_ordenado.index]
        ...
        fig_detalhe = px.bar(
            ...
            labels={'x': 'Quantidade de Candidatos', 'y': 'Fase'},
    ...
    with col4:
        ...
        st.metric("Taxa Fit Cultural", f"{(passaram_fit_cultural/responderam*100 if responderam > 0 else 0):.1f}%")
        st.metric("Taxa Comparecimento", f"{(compareceram_dinamica/passaram_fit_cultural*100 if passaram_fit_cultural > 0 else 0):.1f}%")
        st.metric("Taxa Aprovação Dinâmica", f"{(aprovados_dinamica/compareceram_dinamica*100 if compareceram_dinamica > 0 else 0):.1f}%")
        st.metric("Taxa Final (Entrevista)", f"{(entrevista/total*100):.1f}%")
    
    st.markdown("---")
    st.subheader("Distribuição de Cursos ao Longo do Funil")
    st.info("Mostra como cada curso se comporta em cada etapa do processo seletivo")
    ...
    # Fase 2: Responderam
    df_responderam_curso = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta", na=False)].copy()
    ...
    df_fit_curso = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural", na=False, regex=True)].copy()
    df_fit_curso['Fase_Funil'] = '3. Aprovados Fit'
    fases_curso_analise.append(df_fit_curso)
    
    # Fase 4: Compareceram Dinâmica
    df_compareceram_curso = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica", na=False, regex=True)].copy()
    df_compareceram_curso['Fase_Funil'] = '4. Compareceram'
    fases_curso_analise.append(df_compareceram_curso)
    
    # Fase 5: Aprovados Dinâmica
    df_aprovados_curso = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica|Eliminados da Dinâmica", na=False, regex=True)].copy()
    df_aprovados_curso['Fase_Funil'] = '5. Aprovados Din.'
    fases_curso_analise.append(df_aprovados_curso)
    
    # Fase 6: Entrevista
    df_entrevista_curso = df_filtrado[df_filtrado["Fase atual"] == "Entrevista"].copy()
    ...
    with col_curso1:
        fase_curso_abs = pd.crosstab(
            df_funil_curso["Fase_Funil"],
        ...
        fig3 = px.bar(
            ...
            labels={'value': 'Quantidade', 'variable': 'Curso', 'Fase_Funil': 'Fase'}
        )
    ...
    with col_curso2:
        ...
        fig3b = px.bar(
            ...
            labels={'value': 'Proporção', 'variable': 'Curso', 'Fase_Funil': 'Fase'}
        )
...
with tab2:
    ...
    with col1:
        ...
        raca_counts = df_filtrado["Como você se autodeclara em relação à sua cor ou raça?  "].value_counts().sort_values(ascending=True)
        ...
        fig4 = px.bar(
            ...
            text=[f"{val} ({pct}%)" for val, pct in zip(raca_counts.values, percentuais)],
    ...
    with col2:
        ...
        pronome_counts = df_filtrado["Quais pronomes você utiliza?  "].value_counts()
        fig5 = px.bar(
            x=pronome_counts.index,
            y=pronome_counts.values,
            title="Pronomes Utilizados",
            labels={'x': 'Pronomes', 'y': 'Quantidade'},
    ...
    with col3:
        ...
        lgbtq_counts = df_filtrado["Você se identifica como parte da comunidade LGBTQIAPN+?  "].value_counts()
        
        cores_lgbtq = {'Sim': '#FF6B9D', 'Não': '#4ECDC4', 'Prefiro não responder': '#95E1D3'}
        ...
        fig6 = px.bar(
            ...
            text=[f"{val}<br>({pct}%)" for val, pct in zip(lgbtq_counts.values, percentuais)]
        )
    ...
    with col4:
        ...
        def_counts = df_filtrado["Você possui alguma deficiência, condição ou necessidade específica que a IN Junior deveria considerar para garantir acessibilidade e inclusão?  "].value_counts()
        fig7 = px.bar(
            x=def_counts.index,
            y=def_counts.values,
    ...
    if len(df_entrevista) > 0:
        ...
        with col5:
            raca_entrevista = df_entrevista["Como você se autodeclara em relação à sua cor ou raça?  "].value_counts()
            fig8 = px.bar(
        ...
        with col6:
            ...
            lgbtq_entrevista = df_entrevista["Você se identifica como parte da comunidade LGBTQIAPN+?  "].value_counts()
            pronome_entrevista = df_entrevista["Quais pronomes você utiliza?  "].value_counts()
            ...
            fig8b = px.bar(
                ...
                labels={'x': 'Quantidade', 'y': 'Pronomes'},
    ...
    if len(df_eliminados) > 0:
        ...
        with col7:
            raca_eliminados = df_eliminados["Como você se autodeclara em relação à sua cor ou raça?  "].value_counts()
            fig9 = px.bar(
        ...
        with col8:
            pronome_eliminados = df_eliminados["Quais pronomes você utiliza?  "].value_counts()
            fig9b = px.bar(
    ...
    st.info("Mostra a composição de todos os candidatos que chegaram até cada etapa do funil")
    ...
    df_responderam = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta", na=False)].copy()
    df_responderam['Fase_Funil'] = '2. Responderam'
    fases_analise.append(df_responderam)
    
    # Fase 3: Aprovados Fit Cultural
    df_fit = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural", na=False, regex=True)].copy()
    df_fit['Fase_Funil'] = '3. Aprovados Fit Cultural'
    fases_analise.append(df_fit)
    
    # Fase 4: Compareceram Dinâmica
    df_compareceram = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica", na=False, regex=True)].copy()
    df_compareceram['Fase_Funil'] = '4. Compareceram Dinâmica'
    fases_analise.append(df_compareceram)
    
    # Fase 5: Aprovados Dinâmica
    df_aprovados = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica|Eliminados da Dinâmica", na=False, regex=True)].copy()
    df_aprovados['Fase_Funil'] = '5. Aprovados Dinâmica'
    fases_analise.append(df_aprovados)
    
    # Fase 6: Entrevista
    df_entrevista_funil = df_filtrado[df_filtrado["Fase atual"] == "Entrevista"].copy()
    ...
    with col9:
        fase_raca_processo = pd.crosstab(
            ...
            df_funil_completo["Como você se autodeclara em relação à sua cor ou raça?  "]
        ).reset_index()
        ...
        fig10 = px.bar(
            ...
            labels={'value': 'Quantidade', 'variable': 'Raça/Cor', 'Fase_Funil': 'Fase'}
        )
    ...
    with col10:
        ...
        fig10b = px.bar(
            ...
            labels={'value': 'Proporção', 'variable': 'Raça/Cor', 'Fase_Funil': 'Fase'}
        )
    ...
    with col11:
        fase_lgbtq_processo = pd.crosstab(
            df_funil_completo["Fase_Funil"],
            df_funil_completo["Você se identifica como parte da comunidade LGBTQIAPN+?  "]
        ).reset_index()
        ...
        fig11_lgbtq = px.bar(
            ...
            labels={'value': 'Quantidade', 'variable': 'LGBTQIAPN+', 'Fase_Funil': 'Fase'},
    ...
    with col12:
        ...
        fig11b_lgbtq = px.bar(
            ...
            labels={'value': 'Proporção', 'variable': 'LGBTQIAPN+', 'Fase_Funil': 'Fase'},
            color_discrete_sequence=['#FF6B9D', '#4ECDC4', '#95E1D3']
        )
    ...
    with col13:
        ...
        fig12_pronome = px.bar(
            ...
            barmode='stack',
            title="Distribuição de Identidade de Gênero por Fase (Valores Absolutos)",
            labels={'value': 'Quantidade', 'variable': 'Pronomes', 'Fase_Funil': 'Fase'}
        )
    ...
    with col14:
        ...
        fig12b_pronome = px.bar(
            ...
            labels={'value': 'Proporção', 'variable': 'Pronomes', 'Fase_Funil': 'Fase'}
        )
...
with tab3:
    ...
    with col1:
        ...
        periodo_counts = df_filtrado["Qual o seu período?"].value_counts().sort_index()
        fig11 = px.bar(
    ...
    with col2:
        ...
        aprovacao_curso = df_filtrado.groupby("Curso")["Fase atual"].apply(
            lambda x: (x == "Entrevista").sum() / len(x) * 100 if len(x) > 0 else 0
    ...
    st.info("Mostra como a composição de períodos evolui em cada etapa do processo seletivo")
    ...
    # Fase 2: Responderam
    df_responderam_p = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta", na=False)].copy()
    df_responderam_p['Fase_Funil'] = '2. Responderam'
    fases_periodo.append(df_responderam_p)
    
    # Fase 3: Aprovados Fit Cultural
    df_fit_p = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural", na=False, regex=True)].copy()
    df_fit_p['Fase_Funil'] = '3. Aprovados Fit'
    fases_periodo.append(df_fit_p)
    
    # Fase 4: Compareceram Dinâmica
    df_compareceram_p = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica", na=False, regex=True)].copy()
    df_compareceram_p['Fase_Funil'] = '4. Compareceram'
    fases_periodo.append(df_compareceram_p)
    
    # Fase 5: Aprovados Dinâmica
    df_aprovados_p = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica|Eliminados da Dinâmica", na=False, regex=True)].copy()
    df_aprovados_p['Fase_Funil'] = '5. Aprovados Din.'
    fases_periodo.append(df_aprovados_p)
    
    # Fase 6: Entrevista
    df_entrevista_p = df_filtrado[df_filtrado["Fase atual"] == "Entrevista"].copy()
    ...
    with col_p1:
        ...
        fig_periodo1 = px.bar(
            ...
            labels={'value': 'Quantidade', 'variable': 'Período', 'Fase_Funil': 'Fase'}
        )
    ...
    with col_p2:
        ...
        fig_periodo2 = px.bar(
            ...
            labels={'value': 'Proporção', 'variable': 'Período', 'Fase_Funil': 'Fase'}
        )
...
with tab4:
    ...
    divulgacao_counts = df_filtrado["Como você ficou sabendo do processo seletivo?"].value_counts().sort_values(ascending=True)
    ...
    fig14 = px.bar(
        ...
        labels={'x': 'Número de Candidatos', 'y': 'Canal'},
        text=[f"{val} ({pct}%)" for val, pct in zip(divulgacao_counts.values, percentuais)],
    ...
    st.info("Mostra como cada canal de divulgação se comporta em cada etapa do processo seletivo")
    ...
    # Fase 2: Responderam
    df_responderam_c = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta", na=False)].copy()
    df_responderam_c['Fase_Funil'] = '2. Responderam'
    fases_canal.append(df_responderam_c)
    
    # Fase 3: Aprovados Fit Cultural
    df_fit_c = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural", na=False, regex=True)].copy()
    df_fit_c['Fase_Funil'] = '3. Aprovados Fit'
    fases_canal.append(df_fit_c)
    
    # Fase 4: Compareceram Dinâmica
    df_compareceram_c = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica", na=False, regex=True)].copy()
    df_compareceram_c['Fase_Funil'] = '4. Compareceram'
    fases_canal.append(df_compareceram_c)
    
    # Fase 5: Aprovados Dinâmica
    df_aprovados_c = df_filtrado[~df_filtrado["Fase atual"].str.contains("Falta de Resposta|Fit Cultural|Falta na Dinâmica|Eliminados da Dinâmica", na=False, regex=True)].copy()
    df_aprovados_c['Fase_Funil'] = '5. Aprovados Din.'
    fases_canal.append(df_aprovados_c)
    
    # Fase 6: Entrevista
    df_entrevista_c = df_filtrado[df_filtrado["Fase atual"] == "Entrevista"].copy()
    ...
    with col_c1:
        ...
        fig15 = px.bar(
            ...
            labels={'value': 'Quantidade', 'variable': 'Canal', 'Fase_Funil': 'Fase'}
        )
    ...
    with col_c2:
        ...
        fig16 = px.bar(
            ...
            labels={'value': 'Proporção', 'variable': 'Canal', 'Fase_Funil': 'Fase'}
        )
    ...
    taxa_conversao = df_filtrado.groupby("Como você ficou sabendo do processo seletivo?")["Fase atual"].apply(
        lambda x: (x == "Entrevista").sum() / len(x) * 100 if len(x) > 0 else 0
    ).sort_values(ascending=False)
    ...
    fig17 = px.bar(
        ...
        labels={'x': 'Taxa de Conversão (%)', 'y': 'Canal'},
...
with tab5:
    ...
    with col2:
        ...
        st.metric("Períodos Diferentes", df_filtrado["Qual o seu período?"].nunique())
    
    with col3:
        candidatos_deficiencia = len(df_filtrado[df_filtrado["Você possui alguma deficiência, condição ou necessidade específica que a IN Junior deveria considerar para garantir acessibilidade e inclusão?  "] == "Sim"])
        st.metric("Com Deficiência/Necessidades", candidatos_deficiencia)
        pct_feminino = len(df_filtrado[df_filtrado["Quais pronomes você utiliza?  "] == "Ela/Dela"]) / len(df_filtrado) * 100 if len(df_filtrado) > 0 else 0
...
# Footer
st.markdown("---")
st.markdown("*Dashboard desenvolvido para análise do Processo Seletivo da IN Junior*")
